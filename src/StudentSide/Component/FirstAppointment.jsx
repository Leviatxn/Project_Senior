...
const FirstAppointment = () => {
  ...
  const [appointmentState, setAppointmentState] = useState(null); // เก็บข้อมูลผู้ใช้ทั้งหมด
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        console.log(response.data.is_firstappointment)
        setAppointmentState(response.data.is_firstappointment); // เก็บข้อมูลผู้ใช้ทั้งหมด
    ...
    if (!appointmentInfo.Date || !appointmentInfo.Time) {
      ...
      try {
        const response = await axios.post(
          `http://localhost:5000/addAppointment1/${studentId}`,
          {
    ...
    if (!appointmentData.appointment_date || !appointmentData.appointment_time) {
      ...
      try {
        const response = await axios.put(
          `http://localhost:5000/updateAppointment1/${studentId}`,
          {
          ...
          <form onSubmit={handleSubmit}>
            ...
            <Grid container justifyContent="flex-end">
                ...
                <Button
                    ...
                    variant="contained"
                    color="primary"
                    onClick={()=>(handleEvaluationClick(1))}
                    sx={{
                        mt: 2,
                        width: "181px",
                        backgroundColor: "#00A6A2",
                ...
                <Button
                    ...
                    variant="contained"
                    color="primary"
                    onClick={()=>(handleEvaluationClick(3))}
                    sx={{
                        mt: 2,
                        width: "181px",
                        backgroundColor: "#00A6A2",
