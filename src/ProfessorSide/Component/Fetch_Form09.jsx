...
const Fetch_Form09 = ({ evaluationID }) => {
  ...
  const getMajorName = (major) => {
    switch (major) {
      case 'T12': return "วิศวกรรมคอมพิวเตอร์และสารสนเทศศาสตร์";
      case 'T13': return "วิศวกรรมเครื่องกลและการออกแบบ";
      case 'T14': return "วิศวกรรมไฟฟ้าและอิเล็กทรอนิกส์";
      case 'T17': return "วิศวกรรมอุตสาหการและระบบ";
      case 'T20': return "วิศวกรรมระบบการผลิตดิจิทัล";
      case 'T23': return "วิศวกรรมดิจิทัลและอีเล็กทรอนิกส์อัจฉริยะ";
  ...
  const fetchScores = async (evaluationID) => {
    try {
      ...
      if (response.data) {
        ...
        // ดึงข้อมูลรายงาน
        const reportResponse = await axios.get(`http://localhost:5000/coop_report/${evaluationID}`);
        if (reportResponse.data) {
          setReport({
            ReportTitleTH: reportResponse.data.report_title_th || "",
            ReportTitleENG: reportResponse.data.report_title_eng || "",
            additionalComments: reportResponse.data.additional_comments || ""
          });
        }
      }
    } catch (error) {
  ...
  const renderTableSectionFromData = (section) => {
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      pdf.save(`แบบประเมินผลรายงานนิสิตสหกิจศึกษา${info.studentID}.pdf`);
      ...
      <div ref={pdfRef}>
          ...
          <Paper sx={{ 
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', flex: '1' }}>
              ...
              <h1 className="table-title" style={{ fontWeight: '400', fontSize: '18px', fontFamily: "Noto Sans Thai, sans-serif" }}>
          ...
          <Paper sx={{ padding: 3, marginBottom: 3, boxShadow: 3, borderRadius: 2 }}>
            <Typography variant="9" sx={{ fontWeight: 'bold', color: 'black', mb: 2 }}>
              หัวข้อรายงาน / Report title :
            </Typography>
            <Typography variant="p" sx={{ mb: 3, ml: 5 }}>
              {report.ReportTitleTH || "-"}
            </Typography>
            
            <Typography variant="p" sx={{ px:3,mb: 1, color: '#767676' }}>
              /

            </Typography>
            <Typography variant="p" sx={{ mb: 1, ml: 2 }}>
              {report.ReportTitleENG || "-"}
            </Typography>
          </Paper>

          {/* ส่วนการประเมิน */}
          {evaluationData.map(section => renderTableSectionFromData(section))}
